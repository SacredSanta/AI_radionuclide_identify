...
y_train = model_data["y_train"][:,:,:3]

x_val = model_data["x_val"][:,:,:2]
y_val = model_data["y_val"][:,:,:3]

x_test = model_data["x_test"][:,:,:2]
y_test = model_data["y_test"][:,:,:3]
...
date = "241116_densenet_vfrog_adam" 
foldername = "241116_frogmodel_class8"
csv_logger = tf.keras.callbacks.CSVLogger(filename=f"./2.model/241110_frogmodel/{foldername}/{filename}_{date}.log",
                                          separator=',',
                                          append=False)
weights = tf.keras.callbacks.ModelCheckpoint(filepath=f"./2.model/241110_frogmodel/{foldername}/{filename}_{date}.keras",
...
model = Model
input_data = tf.random.normal([1, 1, 1000, 2])
...
os.makedirs(f"./2.model/{foldername}/predict", exist_ok=True)
...
with open(f"./2.model/241110_frogmodel/{foldername}/{filename}_{date}.txt", 'a') as f:     
...
Model.save(f"./2.model/241110_frogmodel/241116_frogmodel/Model_{filename}__{date}.keras")
