...
source_ = 'cs137' # <-------------------- 개인 변경
distance = 'close' # <-------------------- 개인 변경

rowsize = 1000 # 이미지의 row 개수

starttime = 0 # <-------------------- 개인 변경
finaltime = 50 # data의 마지막 시간    # <-------------------- 개인 변경
...
plt.axvline(x=662, color='white', linestyle='--', linewidth=2)

#%% plot peak column with image----------------------------------------
bin_ = [356, 511, 662]
for i in range(3):
    plt.subplot(2,2,i+1)
    plt.plot(results[:,bin_[i]])
    plt.ylim([0, 200])
plt.subplot(2,2,4)
plt.imshow(results, cmap='hot', vmin=0, vmax=200)
#%% plot columns
ccc = 8
peak = 657
for i in range(ccc):
    plt.subplot(ccc,1,i+1)
    plt.plot(results[:,peak+i])
    plt.ylim([0, 150])
