...
src = src_list[6]  #! -> 바꿔서 사용!
distance = distance_list[2]  #! -> 바꿔서 사용!
...
test.find_peak_idx(range(250,400)) #! -> 바꿔서 사용!
test.modi_spec(real_peak[6])  #! -> 바꿔서 사용!
test.modi_spec_show()
...
def work_1(ii):
    ...
    # (2) 특정 시간 구간에 대한 구현. 
    total_time = 0
    while total_time < 10 or total_time > 20:  #! <----------------------이부분 수정해서 이용 !!
