...
real_peak = [356,     662,     511,    '-',          122,    238,     186]
distance_list = ["close", "35cm", "67cm", "103cm"]

# #* input =======================
# data_title = "240909_nucare"
# thres = "500"
...
#filename = f"{data_title}/data/240909_{location_}_{src}_{counts__}_{interval_}_thres{thres_}_cali{cali}.csv"  #240909
# filename = f"{data_title}/ori/{src}_{distance}_5min.csv"

direc = "../../Data/"
foldername = "240906_arale_indoor_det2/"
foldername2 = "240906_arale_indoor_det2"
files = os.listdir(os.path.join(direc, foldername))
files2 = os.listdir(os.path.join(direc, foldername2))
...
# print(datadir)
datadir_a = os.path.join(direc, foldername, files[4])
datadir_b = os.path.join(direc, foldername, files[0])
print(datadir_b)

#%% 1. make histogram ------------------------------------------------------------
#* datadir = os.path.join(direc, "background_indoor.csv")  # background data
test_a = modi_hist_extract(datadir_a, cali='off')
test_b = modi_hist_extract(datadir_b, cali='off')
import matplotlib.pyplot as plt

plt.plot(test_b.hist)
x_ticks = [i*50 for i in range(0,20)]
x_labels = [50*i for i in range(0,20)]
plt.xticks(ticks=x_ticks, labels=x_labels, fontsize=6)

#%% 1-2. calculate correction coef -----------------------------------------------
test_a.find_peak([250,300])
test_b.find_peak([])
...
b[0] = target[0] / (x[1]-x[0])
b[1] = target[1] / (x[1]-x[0])
...
if fixed:
    ...
    test.find_peak([250, 350]) #! -> 바꿔서 사용!
    test.fix_data(real_peak[2])  #! -> 바꿔서 사용!
    print("test.coef : ", test.scale_factor)
