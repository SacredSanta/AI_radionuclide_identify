...
index = [i for i in range(len(source))]   # source list에서 뽑을 index
#endtime_values = np.linspace(time_[0]+interval, finaltime, rowsize, endpoint=True)
...
def make2d_dataset(count):
    ...
    for idx in final_select:
        ...
        results = Parallel(n_jobs=12, verbose=0)(delayed(onestack_histimage)(i, i+1, fil_dt) for i in range(len(endtime_values)))
        results = np.array(results)
...
for count in range(total_data_count):
    if count % 100 == 0: print(count, " data proceed..")
...
# %% 5. save generated data
date = "240624"
filename = f"{date}_{time_range[0]}to{time_range[1]}sec_{len(index)}source_{total_data_count}_acc{accumulate}"
print(filename)
