...
def radionuclides_densenet(growth_rate:int, nblocks:list):
    ...
    # input
    input_tensor = tf.keras.Input(shape=(1000,3), name="input")
    ...
    x01_p = tf.keras.layers.MaxPool1D(pool_size=7, strides=2, name='x01_p')(x01_c)  # pool size는 행,열    열을 7개씩 pool)
    print("x01_c : ", x01_c.shape)
    print("x01_p : ", x01_p.shape)
    
    
    d01_p = tf.keras.layers.AvgPool1D(pool_size=2, strides=2, name="d01_p")(d01)
    d01_pp = tf.keras.layers.AvgPool1D(pool_size=2, strides=2, name="d01_pp")(d01_p)
    d01_ppp = tf.keras.layers.AvgPool1D(pool_size=2, strides=2, name="d01_ppp")(d01_pp)
    print("d01 : ", d01.shape)
    ...
    d02_p = tf.keras.layers.AvgPool1D(pool_size=2, strides=2, name="d02_p")(d02)
    d02_pp = tf.keras.layers.AvgPool1D(pool_size=2, strides=2, name='d02_pp')(d02_p)
    print("d02 : ", d02.shape)
    ...
    d03_p = tf.keras.layers.AvgPool1D(pool_size=2, strides=2, name="d03_p")(d03)
    ...
    for i in [d01_ppp, d02_pp, d03_p]:
        temp += 1
        d04 = tf.keras.layers.Concatenate(name=f"d04_concate_{temp}")([d04, i])
    ...
    d04 = tf.keras.layers.GlobalAveragePooling1D(name="d04_global")(d04)

    print("GlobalAveragePooling : ", d04.shape)
    

    output_tensor = tf.keras.layers.Dense(num_class, activation='sigmoid', name='last_Dense')(d04)
    model = tf.keras.Model(inputs=input_tensor, outputs=output_tensor)

    return model
#%%

Model = radionuclides_densenet(growth_rate=16, nblocks=[2,4,8,4,4])
...
print(y_val.shape)
#%% 3. CSV Logger -------------------------------------------------------------
date = "241022_densenet1d_vfrog" 
csv_logger = tf.keras.callbacks.CSVLogger(filename=f"./2.model/{filename}_{date}.log",
                                          separator=',',
                                          append=False)
weights = tf.keras.callbacks.ModelCheckpoint(filepath=f"./2.model/{filename}_{date}_ckpt.keras",
...
Model.save(f"./2.model/{filename}_{date}.keras")
...
graph_info = profile(model_fn.get_concrete_function(x).graph, options=profiler_options)
