...
for w in values:
    for d in values:
        for r in values:
            ...
            if product < 0.8:  # 오차 범위 ±0.05
                coef_results.append([round(w,2), round(d,2), round(r,2)])

coef_results = list(map(list, set(map(tuple, coef_results))))
...
init_ = 1.6
interv = 0.1
phis = [round(i*interv + init_, 2) for i in range(0,4,1)]
...
epoch = 40
foldername = f"241113_effinet_combi/{filename}/blocks_arg6/2.9,2.2,0.3"
coefs_combis = [[5.49, 1.0, 0.15],[6.11, 1.0, 0.13]]
phi_flag = 0
