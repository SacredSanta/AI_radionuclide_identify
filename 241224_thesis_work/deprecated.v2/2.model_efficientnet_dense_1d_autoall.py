...
def block(inputs, drop_rate=0., name='',
    ...
    if (id_skip is True and strides == 1 and filters_in == filters_out):
        ...
        if inputs.shape[-1] != x.shape[-1]:
            inputs = layers.Conv1D(x.shape[-1], 1,
                                   padding='same',
                                   use_bias=False,
...
def EfficientNet(width_coef,
    ...
    for (i, args) in enumerate(blocks_args):
        ...
        if i == 0 :
            ...
            args['filters_in'] = round_filters(temp)
        ...
        for j in range(round_repeats(args.pop('repeats'))):
            ...
            x1 = block(x, drop_connect_rate * b / blocks,
            ...
            #x = tf.keras.layers.
            x = tf.keras.layers.Concatenate(axis=2)([x, x1])
            
            args['filters_out'] += round_filters(args['filters_in'])
            b += 1
        filter_info_b.append(args['filters_out'])
        temp = int(np.ceil(args['filters_out'] * 0.5))
...
Model, filter_info_a, filter_info_b = EfficientNet(width_coef=1, 
                                                       depth_coef=1,
                                                       resol_coef=1,
                                                       blocks_args=blocks_arg_2,
                                                       classes=8)
Model.summary()
#%% all-in-one ===================================================
import datetime
...
init_ = 0.5
interv = 0.05
phis = [round(i*interv + init_, 2) for i in range(0,20,1)]
...
epoch = 30
foldername = f"241113_effinet_dense_combi/{filename}/blocks_arg4/"
coefs_combis = [[1,1,1]]
phi_flag = 0
blocks_arg_full = copy.deepcopy(blocks_arg_4)
...
for coefs_combi in coefs_combis:  # <---------- 중간에 끊겼다면 ceofs_combis 에서 indexing, phi 부분 폴더 이름 순서가 섞일 수 있음
    ...
    # callback 지정
    date = f"241110_effinet1d_w{coefs_combi[0]}_d{coefs_combi[1]}_r{coefs_combi[2]}" 
