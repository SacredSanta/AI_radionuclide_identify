...
def radionuclides_densenet(growth_rate:int, nblocks:list, width_coef=1, depth_coef=1, resol_coef=1):
    ...
    # model build
    # input
    input_tensor = tf.keras.Input(shape=(rw, 2), name="input")
...
x_train = model_data["x_train"][:3000,:,:2]
y_train = model_data["y_train"][:3000,:,:3]

x_val = model_data["x_val"][:1000,:,:2]
y_val = model_data["y_val"][:1000,:,:3]

x_test = model_data["x_test"][:1000,:,:2]
y_test = model_data["y_test"][:1000,:,:3]
...
foldername = f"241113_densenet1d_combi/previous/3class"
coefs_combis = [[1,1,1]]
phi_flag = 0
...
for coefs_combi in coefs_combis:
    ...
    test_acc = (np.sum(np.sum( (pred > 0.5) == y_test, axis=1) == 3) / len(y_test) * 100)
